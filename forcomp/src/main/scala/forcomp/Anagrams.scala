...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val unsorted = w groupBy(ch => ch.toLower) map(value => (value._1, value._2.length))
   ...
   *
   *  Note: the resulting value is an occurrence - meaning it is sorte
   *  and has no zero-entries.
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (x foldLeft List[(Char, Int)]()) {
      case (acc: Occurrences, (ch: Char, freq: Int)) => {
        if(!y.toMap.contains(ch))
          acc ++ List((ch, freq))
        else if(freq < y.toMap.apply(ch))
          throw new Error
        else if(freq == y.toMap.apply(ch))
          acc
        else
          acc ++ List((ch, freq - y.toMap.apply(ch)))
      }
    }
  }
