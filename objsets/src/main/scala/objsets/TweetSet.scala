...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    def retweetedAcc(tweet1: Tweet, tweet2: Tweet) =
      if(tweet1.retweets > tweet2.retweets) tweet1 else tweet2
    if(left.isEmpty & right.isEmpty) elem
    else if(left.isEmpty) retweetedAcc(right.mostRetweeted, elem)
    else if(right.isEmpty) retweetedAcc(left.mostRetweeted, elem)
    else retweetedAcc(retweetedAcc(left.mostRetweeted, elem), retweetedAcc(right.mostRetweeted, elem))
  }
